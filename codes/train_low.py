...
logging.info('=======================Training Low Resolution images alone============================')
logging.info('[{}][train_low.py][          lr] : {}'.format(datetime.datetime.now().strftime('%dDT%HH%MM'),str(init_lr)))
...
for epoch in range(100):
    ...
    if (epoch + 1) % 10 > 0 :
        ...
        logging.info('[{}][EPOCH{}][Training] loss : {}'.format(timestamp, epoch+1,"{0:.3f}".format(loss)))
        continue
    ...
    torch.save(net.state_dict(), './stanford_low/teachernet_' + str(epoch) + '_epoch_acc_' + str(acc_validation*100) +'.pt')

torch.save(net.state_dict(), './stanford_low/teachernet.pt')
